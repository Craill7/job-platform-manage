...
import com.ruoyi.recruit.domain.AuditBody;
...
public class JobAuditController extends BaseController {
    ...
    public TableDataInfo list(Jobs jobs) {
        startPage();
        List<Jobs> list = jobsService.selectJobAuditList(jobs);
    ...
    public AjaxResult getInfo(@PathVariable("id") Long id) {
        return AjaxResult.success(jobsService.selectJobsById(id));
    }
    ...
    public AjaxResult reject(@PathVariable Long jobId, @RequestBody AuditBody body) {
        String contact = (body != null) ? body.getOperatorContact() : null;
    ...
    public void export(HttpServletResponse response, Jobs jobs) {
        // 复用查询逻辑
        List<Jobs> list = jobsService.selectJobAuditList(jobs);
        ExcelUtil<Jobs> util = new ExcelUtil<>(Jobs.class);
